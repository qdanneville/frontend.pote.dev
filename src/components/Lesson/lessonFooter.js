...
export const LessonFooter = (props) => {
    return <footer className='flex justify-between items-center py-10 border-t-4 border-dark-blue-900'>
        {
            props?.prev && <div className='flex flex-col justify-end items-end'>
        ...
        {
            props?.next ?
                <Link href={`/cours/${props.nav.course.slug}/${props.next.slug}`}><a className="button" >J'ai terminé cette leçon</a></Link>
                : <Link href={`/cours/${props?.nav?.course?.slug}/`}><a className="button" >J'ai terminé ce cours</a></Link>
        }
        {
            props?.next && <div className='flex flex-col justify-end items-start'>
